...
export const AdjustmentForm = React.memo(({
    ...
    // ── Общие блоки ───────────────────────────────────────────────────────────────
    const typeSelector = (
        <div className={`flex bg-white p-1 rounded-xl shadow-sm border border-slate-200 ${isMobile ? 'w-full' : 'flex-none'}`}>
            {[ProductType.MACHINE, ProductType.PART, ProductType.SERVICE].map(type => (
                <button
                    ...
                    className={`flex-1 flex items-center justify-center gap-1.5 ${isMobile ? 'py-2.5 text-[11px]' : 'px-6 py-2 text-[10px]'} font-black uppercase tracking-widest rounded-lg transition-all ${adjActiveType === type ? (type === ProductType.MACHINE ? 'bg-blue-600' : type === ProductType.PART ? 'bg-orange-600' : 'bg-purple-600') + ' text-white shadow-md' : 'text-slate-400 hover:text-slate-600'}`}
                >
                    {type === ProductType.MACHINE ? <Box size={14}/> : type === ProductType.PART ? <Zap size={14}/> : <Briefcase size={14}/>}
                    {type === ProductType.MACHINE ? 'Станки' : type === ProductType.PART ? 'Запчасти' : 'Услуги'}
                </button>
            ))}
        </div>
    );
    ...
    const productPicker = (
        <div className="relative w-full" ref={prodRef}>
            {!isMobile && <label className="block text-[8px] font-black text-slate-400 uppercase mb-1.5 ml-1 tracking-widest flex items-center gap-1.5"><Search size={10} className="text-blue-500"/> Наименование или Артикул</label>}
            <div className="relative">
                <Search size={16} className="absolute left-4 top-3.5 text-slate-300 pointer-events-none"/>
                <div className={`w-full flex items-center justify-between border border-slate-200 rounded-2xl ${isMobile ? 'py-3.5' : 'py-3'} pl-12 pr-4 text-sm font-bold cursor-pointer bg-white shadow-sm hover:border-blue-300 transition-all`} onClick={() => setIsProdDropdownOpen(!isProdDropdownOpen)}>
    ...
    const machineOptions = selectedProduct?.type === ProductType.MACHINE && selectedProduct.machineConfig && (
        <div className="p-4 bg-white/50 rounded-2xl border border-slate-100 space-y-3 animate-in fade-in duration-300">
            <div className="flex items-center gap-2 mb-2"><Layers size={14} className="text-blue-500"/><span className="text-[10px] font-black text-slate-500 uppercase tracking-widest">Конфигурация лота</span></div>
            <div className={`${isMobile ? 'space-y-3' : 'flex flex-wrap gap-4'}`}>
                {selectedProduct.machineConfig.map(mc => {
                    ...
                    if (!type) return null;
                    return (
                        <div key={mc.typeId} className={isMobile ? 'w-full' : 'flex-1 min-w-[200px]'}>
            ...
            <div className="space-y-4">
                ...
                <div className="space-y-2">
                    {adjActiveType !== ProductType.MACHINE && (
                        <div>
                            <label className="block text-[10px] font-black text-slate-400 uppercase mb-1 tracking-widest flex items-center gap-1.5"><Monitor size={10} className="text-blue-500"/> Совместимость</label>
                            <select className="w-full bg-white border border-slate-200 py-2.5 px-3 rounded-xl text-sm font-bold text-slate-700 outline-none" value={adjMachineFilter} onChange={e => { setAdjMachineFilter(e.target.value); setAdjCategoryFilter('all'); setAdjProductId(''); }}>
                    ...
                    <div>
                        <label className="block text-[10px] font-black text-slate-400 uppercase mb-1 tracking-widest flex items-center gap-1.5"><Tags size={10} className={adjActiveType === ProductType.MACHINE ? "text-blue-600" : "text-orange-500"}/> Категория</label>
                        <select className="w-full bg-white border border-slate-200 py-2.5 px-3 rounded-xl text-sm font-bold text-slate-700 outline-none" value={adjCategoryFilter} onChange={e => { setAdjCategoryFilter(e.target.value); setAdjProductId(''); }}>
                ...
                {/* Товар */}
                <div>
                    <label className="block text-[10px] font-black text-slate-400 uppercase mb-1 tracking-widest">Товар</label>
                    {productPicker}
                </div>
                {/* Кол-во и цены */}
                <div className="grid grid-cols-3 gap-3">
                    <div>
                        <label className="block text-[10px] font-black text-slate-400 uppercase mb-1 tracking-widest text-center">Кол-во</label>
                        <input type="number" inputMode="decimal" className="w-full bg-white border border-slate-200 px-2 py-3 rounded-2xl text-center font-black text-base shadow-sm outline-none" value={adjQty} onChange={e => setAdjQty(parseFloat(e.target.value) || 0)}/>
                    </div>
                    <div>
                        <label className="block text-[10px] font-black text-slate-400 uppercase mb-1 tracking-widest">Себест. (₸)</label>
                        <input type="number" inputMode="decimal" className="w-full bg-white border border-slate-200 p-3 rounded-2xl text-sm font-black text-slate-700 shadow-sm outline-none" value={adjUnitPrice} onChange={e => setAdjUnitPrice(parseFloat(e.target.value) || 0)}/>
                    </div>
                    <div>
                        <label className="block text-[10px] font-black text-emerald-600 uppercase mb-1 tracking-widest">Цена пр. (₸)</label>
                        <input type="number" inputMode="decimal" className="w-full bg-emerald-50 border border-emerald-200 p-3 rounded-2xl text-sm font-black text-emerald-700 shadow-sm outline-none" value={adjSalesPrice} onChange={e => setAdjSalesPrice(parseFloat(e.target.value) || 0)}/>
                    </div>
                </div>
                {machineOptions}
                <button onClick={handleSave} disabled={!adjProductId} className="w-full bg-blue-600 text-white py-4 rounded-2xl font-black text-sm uppercase tracking-widest shadow-lg shadow-blue-200 active:scale-95 transition-all disabled:opacity-30">
                    Провести
                </button>
            </div>
        );
