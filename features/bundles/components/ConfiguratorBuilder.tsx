...
import { Box, ChevronDown, ChevronRight, CheckCircle, Circle, Calculator, Save, ShieldCheck, Tag, PieChart, Settings, Search, X, ListFilter } from 'lucide-react';
...
export const ConfiguratorBuilder: React.FC<ConfiguratorBuilderProps> = ({ onSaved, editingBundle }) => {
  ...
  const bundleTotals = useMemo(() => {
    ...
    let purchaseTotal = machine.basePrice;
    let totalVolume = (machine.packages || []).reduce((acc, p) => acc + p.volumeM3, 0);

    const baseOptionsCost = (machine.machineConfig || []).reduce((acc, configEntry) => {
    ...
    Object.values(config.selectedOptions).flat().forEach(variantId => {
...
}, [machine, config.selectedOptions, optionVariants, optionTypes, exchangeRates]);
                  ...
                  <div key={type.id} className="bg-white rounded-xl border border-slate-200 shadow-sm overflow-hidden transition-all">
                    ...
                    {isExpanded && (
                      <div className="p-2.5 grid grid-cols-2 lg:grid-cols-3 gap-2 bg-slate-50/20">
                        {variants.map(v => 
                            <OptionCard key={v.id} type={type} v={v} machine={machine} configEntry={configEntry} selectedVars={selectedVars} exchangeRates={exchangeRates} access={access} config={config} f={f} optionVariants={optionVariants} />
                        )}
                      </div>
                    )}
                  </div>
      ...
      <div className="w-[320px] flex flex-col bg-white rounded-3xl shadow-2xl border border-slate-200 overflow-hidden flex-none h-full">
        {machine && economy ? (
          <div className="flex flex-col h-full overflow-hidden">
            <div className="p-3.5 bg-slate-900 text-white flex-none flex flex-col items-center justify-center border-b border-white/5">
              ...
              <div className="mt-2 grid grid-cols-3 text-center divide-x divide-white/10">
                <div className="px-2">
...
const OptionCard: React.FC<OptionCardProps> = ({ type, v, machine, configEntry, selectedVars, exchangeRates, access, config, f, optionVariants }) => {

    const priceInMachineCurrency = useMemo(() => {
        const price = configEntry?.priceOverrides[v.id] ?? v.price;
        const vRate = exchangeRates[v.currency] || 1;
        const machineRate = exchangeRates[machine.currency] || 1;
        return price * (vRate / machineRate);
    }, [v, configEntry, exchangeRates, machine]);
    
    const isDefault = (configEntry?.defaultVariantIds || (configEntry?.defaultVariantId ? [configEntry.defaultVariantId] : [])).includes(v.id);
        ...
        <div 
            ...
            {access.canSee('fields', 'option_purchase_price') && (
                <div className="text-[10px] font-black font-mono text-slate-400 mt-1 flex items-center justify-between">
                    <span>
                        {isDefault ? 'Включено' : (priceInMachineCurrency >= 0 ? '+' : '') + f(priceInMachineCurrency) + ` ${machine.currency}`}
                    </span>
                    {v.volumeM3 > 0 && (
                        <span className="text-blue-500 font-bold">+{v.volumeM3.toFixed(2)} м³</span>
                    )}
                </div>
            )}
        </div>
