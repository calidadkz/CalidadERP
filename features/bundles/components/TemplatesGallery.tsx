...
import { Search, RotateCcw, Box, Star, Trash2, ChevronRight, AlignLeft, ChevronDown, ChevronUp, Pencil, LayoutGrid, List, Monitor, Truck, X, Info, Download, Copy, ClipboardList } from 'lucide-react';
...
export const TemplatesGallery: React.FC<TemplatesGalleryProps> = ({ onLoad }) => {
  ...
  const handleCopyOur = (b: Bundle, e: React.MouseEvent) => {
    e.stopPropagation();
    const base = b.baseProductName;
    const variants = b.selectedVariantIds
      .map(vid => optionVariants.find(ov => ov.id === vid)?.name)
      .filter(Boolean) as string[];
    navigator.clipboard.writeText([base, ...variants].join(', '));
    showCopyToast('Скопировано (наши названия)');
  };

  const handleCopySupplier = (b: Bundle, e: React.MouseEvent) => {
    e.stopPropagation();
  ...
  return (
    <div className="flex flex-col h-full overflow-hidden">
      ...
      <div className="flex items-center gap-4 mb-2 px-1 text-[10px] text-slate-400 flex-none">
        <span className="font-black uppercase tracking-widest text-[8px] text-slate-300">Кнопки в строке:</span>
        <span className="flex items-center gap-1"><Copy size={11} className="text-slate-400"/> — скопировать наши названия</span>
        <span className="flex items-center gap-1"><ClipboardList size={11} className="text-slate-400"/> — скопировать для поставщика</span>
        <span className="flex items-center gap-1"><Download size={11} className="text-slate-400"/> — экспорт в CSV</span>
      </div>
      {copyToast && (
        <div className="fixed bottom-6 left-1/2 -translate-x-1/2 z-[9999] bg-slate-800 text-white text-[11px] font-bold px-4 py-2 rounded-xl shadow-xl pointer-events-none animate-in fade-in duration-150">
          {copyToast}
        </div>
      )}
