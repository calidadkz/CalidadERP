                    ...
                    <div className="flex justify-between items-center gap-6">
                        ...
                        <div className="flex flex-1 justify-end gap-3">
                            {activeType !== ProductType.MACHINE && (
                                <div className="w-56"><label className="block text-[8px] font-black text-slate-400 uppercase mb-1 ml-1 tracking-widest flex items-center gap-1.5"><Monitor size={10} className="text-blue-500"/> Совместимость</label><select className="w-full bg-white border border-slate-200 py-2 px-2.5 rounded-xl text-[11px] font-bold text-slate-700 outline-none focus:ring-4 focus:ring-blue-500/10 shadow-sm" value={machineTypeFilter} onChange={e => { setMachineTypeFilter(e.target.value); setCategoryFilter('all'); }}><option value="all">Любое оборудование</option>{machineCategories.map(cat => (<option key={cat.id} value={cat.id}>{cat.name}</option>))}</select></div>
                            )}
                            <div className="w-56"><label className="block text-[8px] font-black text-slate-400 uppercase mb-1 ml-1 tracking-widest flex items-center gap-1.5"><Tags size={10} className={activeType === ProductType.MACHINE ? "text-blue-600" : "text-orange-500"}/> Категория</label><select className="w-full bg-white border border-slate-200 py-2 px-2.5 rounded-xl text-[11px] font-bold text-slate-700 outline-none focus:ring-4 focus:ring-blue-500/10 shadow-sm" value={categoryFilter} onChange={e => setCategoryFilter(e.target.value)}><option value="all">Все категории</option>{displayedCategories.map(cat => (<option key={cat.id} value={cat.id}>{cat.name}</option>))}</select></div>
                        </div>
                    </div>
            ...
            <div className="bg-white border border-slate-100 rounded-2xl overflow-hidden shadow-sm">
                <table className="min-w-full divide-y divide-slate-100">
                    <thead className="bg-slate-50/50 text-[9px] font-bold text-slate-400 uppercase tracking-widest">
                        <tr><th className="px-6 py-3 text-left">Товар (Комплектация)</th><th className="px-4 py-3 text-right w-20">Кол-во</th><th className="px-4 py-3 text-right w-32">Цена за ед.</th><th className="px-6 py-3 text-right w-40">Итого</th><th className="w-12"></th></tr>
                    </thead>
                    <tbody className="bg-white divide-y divide-slate-50">
                        ...
                        {items.length === 0 && (<tr><td colSpan={5} className="py-10 text-center text-slate-300 italic text-xs uppercase font-bold tracking-widest">Список товаров пуст</td></tr>)}
                    </tbody>
                </table>
            </div>

            {isConfiguring && configMachine && (<ConfiguratorModal isOpen={isConfiguring} onClose={() => setIsConfiguring(false)} mode="sales" baseMachine={configMachine} onApply={handleAddItemToOrder} />)}
        </div>
    );
};
